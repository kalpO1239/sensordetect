...
function PhoneCalls() {
  ...
  const filteredCalls = filter === "all" ? calls : calls.filter(call => call.status.toLowerCase() === filter.toLowerCase());
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ backgroundColor: "#f0f0f0" }}>
              ...
              <th style={{ padding: "10px", textAlign: "left", border: "1px solid #ddd" }}>Caller</th>
          ...
          <tbody>
            {filteredCalls.map((call) => (
              <tr key={call.id}>
                <td style={{ padding: "10px", border: "1px solid #ddd" }}>{call.id}</td>
                <td style={{ padding: "10px", border: "1px solid #ddd" }}>
                  <div>{call.caller}</div>
                  <div style={{ fontSize: "14px", color: "#757575" }}>{call.phone}</div>
                </td>
                <td style={{ padding: "10px", border: "1px solid #ddd" }}>{call.timestamp}</td>
                <td style={{ padding: "10px", border: "1px solid #ddd" }}>{call.location}</td>
                <td style={{ padding: "10px", border: "1px solid #ddd" }}>{call.description}</td>
                <td style={{ padding: "10px", border: "1px solid #ddd" }}>
                  <span style={{ 
                    padding: "5px 10px", 
                    borderRadius: "4px", 
                    backgroundColor: 
                      call.status === "Active" ? "#ffecb3" : 
                      call.status === "Dispatched" ? "#e3f2fd" : "#e8f5e9",
                    color: 
                      call.status === "Active" ? "#ff8f00" : 
                      call.status === "Dispatched" ? "#1565c0" : "#2e7d32"
                  }}>
                    {call.status}
                  </span>
                </td>
                <td style={{ padding: "10px", border: "1px solid #ddd" }}>
                  <span style={{ 
                    ...
                    borderRadius: "4px", 
                    backgroundColor: 
                      call.priority === "High" ? "#ffebee" : 
                      call.priority === "Medium" ? "#fff8e1" : "#e8f5e9",
                    color: 
                      call.priority === "High" ? "#c62828" : 
                      call.priority === "Medium" ? "#ff8f00" : "#2e7d32"
                  }}>
                    {call.priority}
                  </span>
                </td>
